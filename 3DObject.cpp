...
Mesh* Model::genMeshObj(aiMesh* mesh)
{
  ...
  Mesh* retObj = new Mesh(vertices, indices, textures, mesh->mName.C_Str());
  return retObj;
}
...
Mesh::Mesh(vector<Vertex> &vertices, vector<GLuint> indices, vector<Texture> textures, const GLchar  *name)
{
...
void Mesh::render(mat4 parentModelMatrix)
{
  ...
  for(GLuint i=0;i<this->textures.size();i++)
  {
    ...
    GLint loc = glGetUniformLocation(programID, (name + number).c_str());
    glUniform1i(loc, i);
